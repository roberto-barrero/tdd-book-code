...
class TestMoney(unittest.TestCase):
    ...
    def testDivision(self):
        originalWon = Money(4002, "KRW")
        expectedWonAfterDivision = Money(1000.5, "KRW")
        self.assertEqual(expectedWonAfterDivision, originalWon.divide(4))
    ...
    def testAdittionOfDollarsAndEuros(self):
        fiveDollars = Money(5, "USD")
        tenEuros = Money(10, "EUR")
        portfolio = Portfolio()
        portfolio.add(fiveDollars, tenEuros)
        expectedValue = Money(17, "USD")
        self.assertEqual(expectedValue, portfolio.evaluate(
            self.bank, "USD"), "%s != %s" % (expectedValue, portfolio.evaluate(self.bank, "USD")))
